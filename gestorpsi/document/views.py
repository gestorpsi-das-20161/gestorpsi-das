...
def document_list(ids, typeDocuments, documents, issuers, states):
    ...
    for i in range(0, len(documents)):
        if (len(documents[i])):
            ...
            if len(typeDocuments[i]):
                td = TypeDocument.objects.get(pk=typeDocuments[i])
            else:
                td = None
            ...
            objs.append(Document(id=ids[i], typeDocument=td, document=documents[
                        i], issuer=iss, state=st))
