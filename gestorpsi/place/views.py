...
def form(request, object_id=None):
    ...
    if object_id:
        object = get_object_or_404(
            Place, pk=object_id, organization=request.user.get_profile().org_active)
...
def save(request, object_id=None):
    ...
    if request.POST.get('place_type') == '1':  # hardcode
        # todos que são matriz
        for x in Place.objects.filter(organization=request.user.get_profile().org_active, place_type__id=1):
...
def room_save(request, object_id=None):
    if object_id:
        object = get_object_or_404(
            Room, pk=object_id, place__organization=request.user.get_profile().org_active)
    else:
        object = Room()
...
def place_order(request, object_id=None):
    ...
    if object.active == True:
        if object.occurrences():
            ...
            for i in object.occurrences():
                occurence_list_html += u'<li><a href="/schedule/events/%s/confirmation/">%s - %s</a></li>' % (
                    i.pk, i, i.event.referral)
                messages.success(request, (_(
                    'You can not disable a place with upcomming occurrences <ul>%s</ul>') % occurence_list_html))
                return HttpResponseRedirect('/place/%s/' % object.id)
        else:
            object.active = False
    else:
    ...
    messages.success(request, ('%s' % (_('Place activated successfully')
                                       if object.active else _('Place deactivated successfully'))))
    return HttpResponseRedirect('/place/%s/' % object.id)
...
def room_order(request, object_id=None):
        ...
        if object.occurrences():
            ...
            for i in object.occurrences():
                occurence_list_html += u'<li><a href="/schedule/events/%s/confirmation/">%s - %s</a></li>' % (
                    i.pk, i, i.event.referral)
            messages.success(request, (_(
                'You can not disable a room with upcomming occurrences <ul>%s</ul>') % occurence_list_html))
            return HttpResponseRedirect('/place/room/%s/' % object.id)
        else:
            object.active = False
    else:
    ...
    messages.success(request, ('%s' % (_('Room activated successfully')
                                       if object.active else _('Room deactivated successfully'))))
    return HttpResponseRedirect('/place/room/%s/' % object.id)
