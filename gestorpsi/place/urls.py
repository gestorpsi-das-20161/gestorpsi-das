...
urlpatterns = patterns('',
                       ...
                       (r'^room/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/$',
                           login_check(room_form)),
                       (r'^room/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/save/$',
                           login_check(room_save)),
                       (r'^room/save/$', login_check(room_save)),
                       (r'^room/page(?P<page>(\d)+)$',
                        login_check(room_list)),  # list objects
                       (r'^room/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/order/$',
                           login_check(room_order)),
                       # quick filter
                       ...
                       (r'^room/filter/(?P<filter>\w+)/deactive/$', login_check(room_list),
                           {'no_paging': True, 'deactive': True}),  # quick search
                       (r'^room/deactive/$', login_check(room_index),
                           {'deactive': True}),  # list objects deactive
                       (r'^room/page(?P<page>(\d)+)/deactive/$',
                           login_check(room_list), {'deactive': True}),  # list objects
                       )
