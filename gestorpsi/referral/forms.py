...
class ReferralForm(forms.ModelForm):
    ...
    group = forms.ModelChoiceField(queryset=ServiceGroup.objects.all(
    ), required=False, widget=forms.Select(attrs={'class': 'extrabig asm', }))
    ...
    ), required=False,  widget=forms.SelectMultiple(attrs={'class': 'extrabig multiple asm', }))
    ...
    priority = forms.ModelChoiceField(queryset=ReferralPriority.objects.all(
    ...
    ), required=False, widget=forms.Select(attrs={'class': 'giant', }))

    class Meta:
        fields = ('client', 'service', 'professional', 'annotation', 'referral', 'annotation',
                  'referral_reason', 'available_time', 'priority', 'impact', 'covenant')
        model = Referral
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            ...
            self.fields['covenant'] = forms.ModelMultipleChoiceField(
                queryset=Covenant.objects.filter(organization=request.user.get_profile(
                ).org_active, active=True, service=ref.service),
                required=False,
            )
        # new register
        except:
            self.fields['covenant'] = forms.ModelMultipleChoiceField(
                queryset=Covenant.objects.filter(
                    organization=request.user.get_profile().org_active, active=True),
                required=False,
            )
            pass
        ...
        if hasattr(self, 'instance') and self.instance.id:
            if self.instance.service.is_group:
                ...
                self.fields['group'].widget.attrs = {
                    'class': 'extrabig asm', 'original_state': self.instance.group.id}
...
class ReferralDischargeForm(forms.ModelForm):
    ...
    class Meta:
        fields = ('reason', 'was_discussed_with_client',
