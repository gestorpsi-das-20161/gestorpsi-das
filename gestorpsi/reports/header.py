...
def header_gen(organization, header_line=True, clinic_info=True):
    # Cannot use image thumbnail because it's too large
    # (PROJECT_ROOT_PATH,pathdir,'.thumb',organization.photo)
    pathdir = '%simg/organization/%s' % (MEDIA_ROOT, organization.id)
    imagefile = os.path.join(PROJECT_ROOT_PATH, pathdir,
                             '.thumb-whitebg', organization.photo)

    class Header(ReportBand):
        height = 3.4 * cm
        borders = {'bottom': False}
        elements = [
            SystemField(expression='%(report_title)s', top=1.9 * cm, left=0, width=BAND_WIDTH,
                        style={'fontName': 'Helvetica', 'fontSize': 12, 'alignment': TA_CENTER}),
            SystemField(expression='Page %(page_number)d of %(page_count)d', top=0.7 * cm,
                        width=BAND_WIDTH, style={'fontName': 'Helvetica', 'fontSize': 8, 'alignment': TA_RIGHT}),
            SystemField(expression='Printed in %(now:%d/%m/%Y %H:%M)s ', top=0.2 * cm, width=BAND_WIDTH,
        ...
        if header_line:
            elements.append(Line(left=0.0 * cm, right=19 * cm,
                                 top=3.3 * cm, bottom=3.3 * cm))
