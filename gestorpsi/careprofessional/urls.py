...
urlpatterns = patterns('',
                       ...
                       url(r'^initial/(?P<initial>[a-zA-Z])/page(?P<page>(\d)+)/$',
                           login_check(list)),
                       url(r'^initial/(?P<initial>[a-zA-Z])/deactive/$',
                       ...
                       url(r'^filter/(?P<filter>\w+)/page(?P<page>(\d)+)/$',
                           login_check(list)),
                       url(r'^filter/(?P<filter>\w+)/deactive/$', login_check(list),
                           {'no_paging': True, 'deactive': True}),
                       url(r'^filter/(?P<filter>\w+)/page(?P<page>(\d)+)/deactive/$',
                           login_check(list), {'deactive': True}),
                       url(r'^student/$', login_check(index), student_index),
                       url(r'^student/page(?P<page>(\d)+)$',
                           login_check(list), student_list),
                       url(r'^student/deactive/$', login_check(index),
                           student_index_deactive),
                       url(r'^student/page(?P<page>(\d)+)/deactive/$',
                       ...
                       url(r'^student/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/$',
                           login_check(form), student_form, name='student_form'),
                       url(r'^student/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/save/$',
                           login_check(save), student_save),
                       url(r'^student/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/order/$',
                           login_check(order), student_save, name='student_order'),
                       url(r'^student/initial/(?P<initial>[a-zA-Z])/page(?P<page>(\d)+)/$',
                           login_check(list), student_list),
                       url(r'^student/initial/(?P<initial>[a-zA-Z])/deactive/$',
                           login_check(list), student_list_deactive),
                       url(r'^student/initial/(?P<initial>[a-zA-Z])/page(?P<page>(\d)+)/deactive/$',
                           login_check(list), student_list_deactive),
                       url(r'^student/filter/(?P<filter>\w+)/$',
                           login_check(list), student_list),
