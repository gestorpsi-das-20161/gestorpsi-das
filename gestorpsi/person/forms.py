...
class CompanyForm(forms.ModelForm):
    cnae_class = forms.CharField(label=_(
        'CNAE Class Code'), required=False, widget=forms.TextInput(attrs={'mask': '9999-9/99'}))
...
class CompanyClientForm(forms.ModelForm):
    ...
    def save(self, request, client, *args, **kwargs):
        company_client = super(CompanyClientForm, self).save(
        ...
        if not request.POST.get('client_id'):
            ...
            client_related = Client(
                person=person, idRecord=request.user.get_profile().org_active.last_id_record + 1)
            client_related.save()
            ...
            if not request.POST.get('client_id') in [i.client.id for i in client.person.company.companyclient_set.all()] and not request.POST.get('active'):
                company_client.active = True
            client_related = Client.objects.get(pk=request.POST.get(
                'client_id'), person__organization=request.user.get_profile().org_active)
