...
def index(request):
    ...
    form = ReportForm(request.user.get_profile().org_active.created(
    ), datetime.now(), request.user.get_profile().org_active)
...
def report_date(request):
    ...
    date_start, date_end = Report().set_date(request.user.get_profile().org_active,
                                             request.GET.get('date_start'), request.GET.get('date_end'))
...
def admission_data(request, template='report/report_table.html'):
    ...
    data, chart_url, date_start, date_end = Report().get_admissions_range(request.user.get_profile(
    ).org_active, request.GET.get('date_start'), request.GET.get('date_end'), request.GET.get('accumulated'))
...
def referral_data(request, template='report/report_table.html'):
    ...
    """
    data, date_start, date_end, service = Report().get_referral_range(request.user.get_profile().org_active, request.GET.get(
        'date_start'), request.GET.get('date_end'), request.GET.get('service'), request.GET.get('accumulated'))
...
def receive_data(request, template='report/report_receive.html'):
    ...
    data, colors, date_start, date_end, list_receive, total_receive = Report().get_receive_(request.user.get_profile().org_active, request.GET.get('date_start'), request.GET.get(
        'date_end'), request.GET.get('professional'), request.GET.get('receive'), request.GET.get('service'), request.GET.get('pway'), request.GET.get('covenant'))
...
def event_data(request, template='report/report_event.html'):
    ...
    event
    """
    data, lines, date_start, date_end, sch_list, total_events = Report().get_event_(request.user.get_profile().org_active, request.GET.get('date_start'),
                                                                                    request.GET.get('date_end'), request.GET.get('professional'), request.GET.get('service'), request.GET.get('status'), request.GET.get('accumulated'))
...
def report_client_list(request, report_class, view, filter):
    ...
    date_start, date_end = report.set_date(
        organization, request.GET.get('date_start'), request.GET.get('date_end'))

    verbose_name, object_list, organization_total = report_class.objects.clients(
        request.user, date_start, date_end, view, filter, request.GET.get('service'))
...
def demographic_data(request, view='admission'):
    ...
    if range:
        demographic = ReportAdmission.objects_demographic.all(
            organization, [i.client.id for i in range])
...
def report_save(request, form_class=None, view=None, template='report/report_save_form.html'):
    ...
    if request.method == 'POST':
        ...
        if save_form.is_valid():
            data = save_form.save(
                request.user, request.user.get_profile().org_active)
    ...
    date_start, date_end = report.set_date(request.user.get_profile(
    ).org_active, request.GET.get('date_start'), request.GET.get('date_end'))

    # financial / revenues
    if view == 'receive':
        form = form_class(date_start=date_start, date_end=date_end, service=request.GET.get('service'),  professional=request.GET.get(
            'professional'), pway=request.GET.get('pway'), receive=request.GET.get('receive'), covenant=request.GET.get('covenant'))
    # admission / referral
    else:
        form = form_class(date_start=date_start, date_end=date_end, service=Service.objects.get(organization=request.user.get_profile(
        ).org_active, pk=request.GET.get('service')) if request.GET.get('service') else None, accumulated=request.GET.get('accumulated'))
...
def report_del(request, object_id, undelete=False):
    ...
    """
    obj = get_object_or_404(ReportsSaved, id=object_id, user=request.user,
                            organization=request.user.get_profile().org_active)
...
def report_empty(request):
    ...
    """
    ReportsSaved.objects.filter(trash=True, user=request.user,
                                organization=request.user.get_profile().org_active).delete()
...
def report_export(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('view') == '1':  # admission
            ...
            view = 'admission'
            data = Report().get_admissions_range(org_active, request.POST.get('date_start'),
                                                 request.POST.get('date_end'), request.POST.get('accumulated'))

            if request.POST.get('clients'):
                report_clients = ReportAdmission.objects.clients_all(
                    request.user, request.POST.get('date_start'), request.POST.get('date_end'))
        ...
        if request.POST.get('view') == '2':  # referral
            ...
            if request.POST.get('service'):
                sub_title = u'%s %s' % (_('Service'), Service.objects.get(
                    organization=request.user.get_profile().org_active, pk=request.POST.get('service')))
            view = 'referral'
            data = Report().get_referral_range(org_active, request.POST.get('date_start'),
                                               request.POST.get('date_end'), request.POST.get('service'), request.POST.get('accumulated'))

            if request.POST.get('clients'):
                report_clients = ReportReferral.objects.clients_all(
                    request.user, request.POST.get('date_start'), request.POST.get('date_end'))
        ...
        if request.POST.get('view') == '3':
            ...
            data, colors, date_start, date_end, list_receive, total_receive = Report().get_receive_(org_active, request.POST.get('date_start'), request.POST.get(
                'date_end'), request.POST.get('professional'), request.POST.get('receipt_status'), request.POST.get('service'), request.POST.get('payment_way'), request.POST.get('cove'))
            ...
            IMG_PREFIX = '/media/' if int(request.POST.get('format')) == 1 else MEDIA_ROOT.replace(
                '\\', '/') + '/'  # this a path bug fix. format == 1 (html)
