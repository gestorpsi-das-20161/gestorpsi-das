...
urlpatterns = patterns('',
                       ...
                       url(r'^messages/chat/postmessage/(?P<messagetopic_id>\d+)/$',
                           login_check(chat_message)),
                       url(r'^referral/(?P<referral_id>\d+)/messages/topic/(?P<topic_id>\d+)/newmessage$',
                           login_check(new_topic_message)),
                       url(r'^referral/(?P<referral_id>\d+)/messages/chat/(?P<scheduleoccurrence_id>\d+)$',
                           login_check(chat_messages_history)),  # referral topic messages
                       url(r'^referral/(?P<referral_id>\d+)/messages/topic/(?P<topic_id>\d+)$',
