...
AUX = os.path.abspath(os.path.join(os.path.dirname(
    os.path.abspath(__file__)), os.path.pardir))
MANAGE = os.path.join(AUX, 'manage.py')

# varre os processos em busca de alguma instancia do django-celery sendo
# executado pelo meu projeto
saida = os.popen(
    "ps -ef | grep -i 'gestorpsi/manage.py celeryd' | grep -v grep | awk '{print $2}'").read()
