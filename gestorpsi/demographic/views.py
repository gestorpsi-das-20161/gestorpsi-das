...
def education_save(request, object_id):
    ...
    if hasattr(object, 'educationallevel'):
        education_form = EducationalLevelForm(
            request.POST, instance=object.educationallevel)
    else:
        education_form = EducationalLevelForm(
            request.POST, instance=EducationalLevel())
...
def occupation(request, object_id, occupation_id=0):
    ...
    occupation_object = get_object_or_None(
        Profession, id=occupation_id) or Profession()
