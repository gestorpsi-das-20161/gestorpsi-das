        ...
        if 'file' in request.FILES:
            ...
            try:
                if file.content_type in ['image/jpeg', 'image/png', 'image/gif']:
                    ...
                    # put background
                    bg = Image.new('RGBA', (116, 134),
                                   (0, 0, 0, 0))  # transparent
                    bg_white = Image.new(
                        'RGBA', (116, 134), (255, 255, 255, 255))  # white
                    W, H = bg.size
                    w, h = img.size
                    xo, yo = (W - w) / 2, (H - h) / 2
                    ...
                    bg_white.save('%s/.thumb-whitebg/%s' %
                                  (pathdir, filename), "PNG")
...
def attach_form(request, object_id=None, referral_id=None, attach_id=None):
    ...
    attach_list = ReferralAttach.objects.filter(
        referral=referral_id, referral__service__organization=request.user.get_profile().org_active)
    object = Client.objects.get(
        pk=object_id, person__organization=request.user.get_profile().org_active)
    ...
    # update
    if attach_id:
        attach = ReferralAttach.objects.get(
            pk=attach_id, referral=referral_id, referral__service__organization=request.user.get_profile().org_active)
        attach_list = ReferralAttach.objects.filter(
            referral=referral_id, referral__service__organization=request.user.get_profile().org_active).exclude(id=attach.id)
