...
urlpatterns = patterns('',
                       ...
                       (r'^(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/save/$', login_check(
                           save)),
                       # save device from device_mini form
                       (r'^save_device/$', login_check(save_device)),
                       (r'^type/$', login_check(index_type)),  # list types
                       (r'^type/page(?P<page>(\d)+)$',
                        login_check(list_types)),  # list type objects
                       # edit type form
                       (r'^type/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/$',
                           login_check(form_type)),
                       # save type
                       (r'^type/(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/save/$',
                           login_check(save_device)),
                       ...
                       # DEACTIVE
                       (r'^initial/(?P<initial>[a-z])/page(?P<page>(\d)+)/deactive/$',
                           login_check(list), {'deactive': True}),  # quick filter
                       (r'^initial/(?P<initial>[a-z])/deactive/$',
                           login_check(list), {'deactive': True}),  # quick filter
