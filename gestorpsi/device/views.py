...
def list(request, page=1, initial=None, filter=None, deactive=False):
    ...
    if deactive:
        object = DeviceDetails.objects.deactive(
            request.user.get_profile().org_active)
    else:
        object = DeviceDetails.objects.active(
            request.user.get_profile().org_active)
...
def form(request, object_id=None):
    object = get_object_or_404(DeviceDetails, pk=object_id, device__organization=request.user.get_profile(
    ).org_active) if object_id else DeviceDetails()
...
def form_type(request, object_id=None):
    object = get_object_or_404(
        Device, pk=object_id, organization=request.user.get_profile().org_active)
...
def save_device(request, object_id=None):
    object = get_object_or_404(Device, pk=object_id, organization=request.user.get_profile(
    ).org_active) if object_id else Device()
...
def save(request, object_id=None):
    ...
    device_details = get_object_or_404(DeviceDetails, pk=object_id, device__organization=request.user.get_profile(
    ).org_active) if object_id else DeviceDetails()
    ...
    device_details.lendable = get_visible(
        request, request.POST.get('lendable'))
    device_details.device = get_object_or_404(
        Device, pk=request.POST.get('select_device'))
    ...
    if request.POST['select_restriction_type'] == '2':
        device_details.prof_restriction = Profession.objects.get(
            pk=request.POST.get('professional_area'))
    ...
    try:
        device_details.place = get_object_or_404(
            Place, pk=request.POST.get('place_associated'))
    except:
        device_details.place = None
    if mobility == '1':
        try:
            device_details.room = get_object_or_404(
                Room, pk=request.POST.get('room_associated'))
        except:
...
def order(request, object_id=None):
    object = get_object_or_404(DeviceDetails, pk=object_id,
                               device__organization=request.user.get_profile().org_active)

    if object.active == True:
        upcoming_occurrences = object.scheduleoccurrence_set.filter(end_time__gt=datetime.now()).exclude(
            occurrenceconfirmation__presence=4).exclude(occurrenceconfirmation__presence=3)
        if len(upcoming_occurrences):
            messages.success(request, _(
                'Sorry, you can not disable a device with upcoming occurrence(s). Total upcoming occurrences %s' % len(upcoming_occurrences)))
    ...
    messages.success(request, ('%s' % (_('Device activated successfully')
                                       if object.active else _('Device deactivated successfully'))))
