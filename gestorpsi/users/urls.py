...
urlpatterns = patterns('',
                       ...
                       (r'^(?P<obj>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/savepwd/$',
                           login_check(update_pwd)),
                       (r'^(?P<object_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/setformuser/$',
                           login_check(set_form_user)),
                       (r'^(?P<obj>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/saveemail/$',
                           login_check(update_email)),
                       (r'^(?P<profile_id>(\d)+)/order/$', login_check(order)),
                       #url(r'^(?P<profile_id>(\d)+)/delete/$', login_check(delete), name='user_delete'),
                       # check username
                       (r'^check/(?P<user>.*)/$',
                        login_check(username_is_available)),
                       # DEACTIVE
                       (r'^deactive/$', login_check(index),
                        {'deactive': True}),
                       (r'^page(?P<page>(\d)+)/deactive/$',
