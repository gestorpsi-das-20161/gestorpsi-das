...
class FamilyForm(forms.ModelForm):
    name = forms.CharField(
        required=True, widget=forms.TextInput(attrs={'class': 'big'}))
    ...
    def save(self, request, client, *args, **kwargs):
        ...
        if request.POST.get('client_id'):  # existing client
            client_related = Client.objects.filter(pk=request.POST.get(
                'client_id'), person__organization=request.user.get_profile().org_active)[0]
