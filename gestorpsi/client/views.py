...
def _access_check_referral_write(request, referral=None):
    ...
    # check if user is professional and not admin or secretary. if it's true,
    # check if professional has referral with this customer
    ...
    if request.user.groups.filter(name='professional') or request.user.groups.filter(name='student'):
        ...
        # lets check if request.user (professional) is responsible for this
        # referral service
...
def index(request, deactive=False):
    # Test if clinic administrator has registered services before access
    # client page.
    ...
    if not Service.objects.filter(active=True, organization=request.user.get_profile().org_active).count():
        msg = _("There's no Service created yet. Please, create one before access Client, ") + \
            " <a href='/service/add'> clique aqui.</a>"
...
def list(request, page=1, initial=None, filter=None, no_paging=False, deactive=False, retrn=False):
    ...
    user = request.user
    object_list = Client.objects.from_user(
        user, 'deactive' if deactive else 'active')
    ...
    if subscribed:
        if not request.GET.get('service'):
            object_list = object_list.filter(
                referral__service__isnull=False).distinct()
        else:
    ...
    if discharged:
        if not request.GET.get('service'):
            object_list = object_list.filter(
                referral__referraldischarge__isnull=False).distinct()
        else:
            for c in object_list:
    ...
    if queued:
        ...
        object_list = object_list.filter(
            referral__queue__isnull=False).distinct()

        url_extra += '&queued=%s' % queued

    # nooccurrences
    nooccurrences = True if request.GET.get(
        'nooccurrences') == "true" else False
    if nooccurrences:
        nooccurrences = request.GET.get('nooccurrences')
        object_list = object_list.filter(
            referral__occurrence__isnull=True).distinct()
...
def add_company(request, object_id=''):
    object = Client() if not object_id else get_object_or_404(Client, pk=object_id,
...
def referral_plus_form(request, object_id=None, referral_id=None):
    ...
    referral_form.fields['professional'].choices = [(p.pk, '%s %s' % (p.person.name, '' if not p.is_student else _(
        'Student'))) for p in CareProfessional.objects.filter(active=True, person__organization=request.user.get_profile().org_active)]
    referral_form.fields[
        'referral'].queryset = Referral.objects.filter(client=object)
    referral_form.fields['service'].queryset = Service.objects.filter(
        active=True, organization=request.user.get_profile().org_active)
    referral_form.fields['client'].queryset = Client.objects.filter(
        person__organization=request.user.get_profile().org_active.id, active=True)
...
def referral_form(request, object_id=None, referral_id=None):
        ...
        if form.is_valid():
            ...
            if data.service.active:
                ...
                if request.POST.get('indication'):
                    ...
                    indication.indication_choice = IndicationChoice.objects.get(
                        pk=request.POST.get('indication'))
                    indication.referral_organization = get_object_or_None(
                        Organization, id=request.POST.get('indication_organization'))
                    indication.referral_professional = get_object_or_None(
                        CareProfessional, id=request.POST.get('indication_professional'))
                ...
                GroupMembers.objects.filter(client=Client.objects.get(
                    pk=object_id, person__organization=request.user.get_profile().org_active), referral=data).delete()
                if data.service.is_group:
                    group = get_object_or_404(ServiceGroup, pk=request.POST.get(
                        'group'), service__organization=request.user.get_profile().org_active)
                    gm = GroupMembers(
                        group=group, client=object, referral=data)
                    gm.save()
    ...
    if referral.id:
        ...
        form.fields['professional'].choices = [(p.pk, '%s %s' % (p.person.name, '' if not p.is_student else _('(Student)'))) for p in CareProfessional.objects.filter(
            active=True, person__organization=request.user.get_profile().org_active, prof_services=referral.service)]
    ...
    if referral.group:
        ...
        form.fields['group'].queryset = ServiceGroup.objects.filter(
            service__organization=request.user.get_profile().org_active).filter(service=referral.service)
    else:
        form.fields['group'].queryset = ServiceGroup.objects.filter(
        ...
        if form.is_valid():
            ...
            if request.POST.get('group'):
                ...
                for c in request.POST.getlist('client'):
                    gm = GroupMembers(group=group, client=Client.objects.get(
                        pk=object_id, person__organization=request.user.get_profile().org_active), referral=object)
                    gm.save()
        # else:
            # print form.errors
...
def referral_save(request, object_id=None, referral_id=None):
    if request.method == 'POST':
        try:
            referral = Referral.objects.get(pk=referral_id, service__organization=request.user.get_profile(
            ).org_active, referraldischarge__isnull=True)
            if not _access_check_referral_write(request, referral):
                return render_to_response('403.html', {'object': _("Oops! You don't have access for this service!"), }, context_instance=RequestContext(request))
            form = ReferralForm(request, request.POST, instance=referral)
        except:
...
def referral_discharge_form(request, object_id=None, referral_id=None, discharge_id=None):
    ...
    if referral.on_queue():
        messages.success(request, _(
            'Sorry, you can not discharge a queued referral. Remove it from queue first before to continue'))
    ...
    if referral.upcoming_occurrences().count() == 0 or instance:
        if request.method == 'POST':
            if not instance:
                form = ReferralDischargeForm(
                    request.POST, initial=dict(client=object, referral=referral))
            else:
                form = ReferralDischargeForm(request.POST, instance=instance, initial=dict(
                    client=object, referral=referral))
            ...
            if form.is_valid():
                ...
                if not instance:
                    messages.success(request, _(
                        'Client discharged successfully'))
                else:
                    messages.success(request, _(
                        'Referral discharge updated successfully'))
            ...
            if not instance:
                form = ReferralDischargeForm(
                    initial=dict(client=object, referral=referral))
            else:
                form = ReferralDischargeForm(
                    instance=instance, initial=dict(client=object, referral=referral))
...
def save(request, object_id=None, is_company=False):
    ...
    if object_id:
        ...
        org = get_object_or_404(
            Organization, pk=user.get_profile().org_active.id)
    ...
    if is_company:
        company_form = CompanyForm(request.POST) if not object_id else CompanyForm(
            request.POST, instance=object.person.company)
    ...
    a.referral_choice_id = AdmissionChoice.objects.all().order_by('weight')[
        0].id
...
def client_print(request, object_id=None):
    ...
    have_ehr_read_perms = False if not _access_ehr_check_read(
        request, object) else True

    if request.POST:

        referral = object.referral_set.filter(
            pk__in=request.POST.getlist('referral'))
        print_schedule = None if not request.POST.get('schedule') else True
        print_demographic = None if not request.POST.get(
            'demographic') else True
        print_ehr = None if not request.POST.get('ehr') else True
        signed_professional_responsible = None if not request.POST.get(
            'signed_professional_responsible') else True
        signed_professionals = None if not request.POST.get(
            'signed_professionals') else True
        signed_organization_reponsibles = None if not request.POST.get(
            'signed_organization_reponsibles') else True

        company_related_clients = []
        if object.is_company():
            company_related_clients = CompanyClient.objects.filter(
                company__person__client=object, company__person__organization=request.user.get_profile().org_active)
...
def organization_clients(request):
    ...
    if not request.GET.get('include_deactivated'):
        clients = Client.objects.from_user(
            request.user, 'active').order_by('person__name')
    else:
        clients = Client.objects.from_user(
            request.user).order_by('person__name')

    if request.GET.get('q'):
        clients = clients.filter(
            person__name__istartswith=request.GET.get('q'))
...
def order(request, object_id=''):
    ...
    if not object.is_active():
        ...
        if Referral.objects.charged().filter(client=object).count() == 0:
            ...
            messages.success(request, _(
                'Sorry, you can not deactivate a client with registered referral'))
            url += '?clss=error'
...
def schedule_daily(
    ...
    **params
    ...
    referral = get_object_or_404(Referral, pk=request.GET.get(
        'referral'), service__organization=request.user.get_profile().org_active)
    ...
    if referral.on_queue():
        messages.success(request, _(
            'Sorry, you can not book a queued client. Remove it first from queue before continue'))
    ...
    if Place.objects.filter(organization=request.user.get_profile().org_active, place_type__id=1):
        place = Place.objects.filter(organization=request.user.get_profile(
        ).org_active, place_type__id=1)[0]  # place type = matriz
    else:
        place = Place.objects.filter(
            organization=request.user.get_profile().org_active, )[0]  # first
...
def occurrence_view(request, object_id=None, occurrence_id=None):
    occurrence = get_object_or_404(ScheduleOccurrence, pk=occurrence_id,
                                   event__referral__service__organization=request.user.get_profile().org_active)
    ...
    return occurrence_confirmation_form(request,
                                        ...
                                        client_id=object_id,
                                        redirect_to='/client/%s/referral/%s/' % (
                                            object_id, occurrence.event.referral.id)
                                        )
...
def referral_occurrences_action(request, object_id=None, referral_id=None):
    ...
    for x in request.POST.getlist('list_occurrence'):
        ...
        oc = get_object_or_404(
            ScheduleOccurrence, pk=x, event__referral__organization=request.user.get_profile().org_active)
    ...
    if msg:
        messages.success(request, _(
            u'Ocorrência salvo com sucesso! Total de %s' % c))
...
def referral_queue(request, object_id='',  referral_id=''):
    ...
    if referral.on_queue():
        messages.success(request, _(
            'Error adding to queue! Referral is already queued'))
        return HttpResponseRedirect('/client/%s/referral/%s/?clss=error' % (object.id, referral.id))

    if referral.upcoming_occurrences():
        messages.success(request, _(
            'Error adding to queue! Referral have upcoming occurrences'))
        return HttpResponseRedirect('/client/%s/referral/%s/?clss=error' % (object.id, referral.id))
...
def referral_queue_save(request, object_id='',  referral_id=''):
    ...
    if referral.upcoming_occurrences():
        messages.success(request, _(
            'Subscript in the queue not is possible. Referral have upcoming occurrences'))
        return HttpResponseRedirect('/client/%s/referral/%s/?clss=error' % (object.id, referral.id))
...
def referral_ext_save(request, object_id='', referral_id=''):
    ...
    if form.is_valid():
        ...
        referral_ext.professional = get_object_or_None(
            CareProfessional, pk=request.POST.get('professional'))
        referral_ext.organization = get_object_or_None(
            Organization, pk=request.POST.get('organization'))
        referral_ext.save()
    # else:
        # print form.errors
...
def family_form(request, object_id=None, relation_id=None):
    ...
    if request.method == 'POST':
        form = FamilyForm(
            request.POST, instance=relation) if relation_id else FamilyForm(request.POST)
        if form.is_valid():
            if not relation_id:
                data = form.save(request, object)
            else:
                relation.responsible = True if request.POST.get(
                    'responsible') else False
                relation.relation_level = None if not request.POST.get(
                    'relation_level') else request.POST.get('relation_level')
        ...
        if relation_id:
            ...
            form.fields['name'].widget = forms.TextInput(
                attrs={'readonly': 'readonly', 'class': 'extrabig'})
...
def company_related(request, object_id=None):
    ...
    clients = CompanyClient.objects.filter(
        company__person__client=object, company__person__organization=request.user.get_profile().org_active)
...
def company_related_form(request, object_id=None, company_client_id=None):
    ...
    if request.method == 'POST':
        if request.POST.get('client_id') in [i.client.pk for i in object.employees()]:
            messages.success(request, _(
                'Employee already registered on this company'))
        ...
        if form.is_valid():
            ...
            messages.success(request, _(
                'Related client added successfully to this company'))
            return HttpResponseRedirect('/client/%s/company_clients/' % object.id)
        else:
