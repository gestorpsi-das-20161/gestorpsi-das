...
class BradescoBilletData(models.Model):
    inscription_default_value = models.DecimalField(
        max_digits=19, decimal_places=10, default=INSCRIPTION_DEFAULT_VALUE)
    inscription_default_value.verbose_name = _("Default inscription tax")
    inscription_default_value.help_text = _(
        "Value charged on the user to make their inscription on the system.")

    default_payment_day = models.PositiveIntegerField(
        validators=[MaxValueValidator(28), MinValueValidator(1)], default=10)
    default_payment_day.verbose_name = _("Default payment day")
    default_payment_day.help_text = _(
        "The default day in which the billets have to be paid by all the organizations (if they define another one that one will be preferred).")

    default_second_copy_days = models.PositiveIntegerField(
        validators=[MaxValueValidator(28), MinValueValidator(1)], default=7)
    default_second_copy_days.verbose_name = _("Second copy tolerance time")
    default_second_copy_days.help_text = _(
        "The default amount of days that will be added to a second copy of a billet.")
    ...
    cedente_cnpj.widget = forms.TextInput(
        attrs={"mask": "99.999.999/9999-99", })
    ...
    sacadoravalista_cnpj = models.CharField(
        max_length=20, null=True, blank=True)
    sacadoravalista_cnpj.widget = forms.TextInput(
        attrs={"mask": "99.999.999/9999-99", })
    ...
    enderecosacaval_localidade = ChainedForeignKey(
        City, chained_field='enderecosacaval_uf', chained_model_field='state', null=True, blank=True)
    enderecosacaval_localidade.verbose_name = _("City")
    enderecosacaval_localidade.help_text = _(
        "City where the the guarantor lives.")
    enderecosacaval_cep = models.CharField(
    ...
    contabancaria_numerodaconta = models.CharField(
        max_length=7, null=False, blank=False)
    contabancaria_numerodaconta.widget = forms.TextInput(
        attrs={"mask": "999999", })
    ...
    contabancaria_numerodaconta_digito = models.CharField(
        max_length=1, null=False, blank=False)
    contabancaria_numerodaconta_digito.widget = forms.TextInput(attrs={
                                                                "mask": "9", })
    ...
    contabancaria_agencia = models.CharField(
    ...
    contabancaria_agencia_digito = models.CharField(
        max_length=1, null=False, blank=False)
    contabancaria_agencia_digito.widget = forms.TextInput(
        attrs={"mask": "9", })
    ...
    titulo_nossonumero = models.CharField(
        max_length=11, null=True, blank=True, default=1, editable=False)
    ...
    titulo_nossonumero.help_text = _(
        "Number registered in Bradesco to register the receiving of billets.")

    titulo_digitodonossonumero = models.CharField(
        max_length=1, null=True, blank=True, default=1, editable=False)
...
class ReturnFile(models.Model):
    date_import = models.DateTimeField(default=datetime.today, verbose_name=_(
        "Date of this file"), blank=True, null=True)
    arquivo_retorno = models.FileField(
        upload_to='arquivos_retorno', verbose_name=_("Return file"))
    banco = models.CharField(_('Bank'), max_length=20, null=True,
                             blank=True, choices=BANK_CHOICES, default='bradesco')
    # numero registros
    # numero linhas
