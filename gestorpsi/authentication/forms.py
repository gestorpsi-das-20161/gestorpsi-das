...
class AuthenticationRegistrationForm(RegistrationRegistrationForm):
    name = forms.CharField(label=_('Name'), help_text=_(
        'Responsible professional name'))
    email = forms.EmailField(
        label=_('Email'), help_text=_('Your email address'))
    password1 = forms.CharField(widget=forms.PasswordInput(
        attrs=attrs_dict, render_value=False), label=_('Password'), help_text=_('Choice one password'))
    password2 = forms.CharField(widget=forms.PasswordInput(attrs=attrs_dict, render_value=False), label=_(
        'Password (again)'), help_text=_('Type password again'))
    username = forms.CharField(
        label=_('Username'), help_text=_('Choice one unique identifier'))
    organization = forms.CharField(
        label=_('Organization'), help_text=_('Name of your organization'))
    shortname = forms.CharField(
        label=_('Short Name'), help_text=_('Shortname of your organization'))
    ...
    def save(self, send_email=True):
        ...
        default_place = Place.objects.create(  # create default place
            ...
            place_type=PlaceType.objects.get(
                description='Matriz'),  # mandatory field
        ...
        person = Person.objects.create(
            name=self.cleaned_data['name'], user=user)
        person.organization.add(organization)
        profile.org_active = organization  # set org as active
        profile.temp = self.cleaned_data[
            'password1']      # temporary field (LDAP)
        ...
        try:
            ...
            Role.objects.create(
                profile=profile, organization=organization, group=admin_role)
        ...
        try:
            ...
            Role.objects.create(
                profile=profile, organization=organization, group=professional_role)
...
class RegistrationForm(AuthenticationRegistrationForm):
    ...
    plan = forms.ModelChoiceField(label=_('Access Plan'), help_text=_(
        'Choice one access plan'), queryset=Plan.objects.filter(active=True))
    phone = forms.CharField(max_length=15, label=_('Phone Number'), help_text=_(
        'Enter your phone number with area code here'), widget=forms.TextInput(attrs={'mask': '(99) 9999-9999?9', }))
    cpf = fields.CPFField(label=_('CPF Number'), help_text=_(
        'Enter your CPF number here'), widget=forms.TextInput(attrs={'mask': '999.999.999-99', }))
    address = forms.CharField(max_length=255, label=_(
        'Address Street'), help_text=_('Enter your address here'))
    address_number = forms.CharField(max_length=30, label=_(
        'Address Number'), help_text=_('Enter your address number here'))
    zipcode = forms.CharField(max_length=30, label=_('ZIP Code'), help_text=_(
        'Enter your ZIP Code here'), widget=forms.TextInput(attrs={'mask': '99999-999'}))
    state = forms.ModelChoiceField(label=_('State/Region'), help_text=_('Enter your state/region here'),
                                   queryset=State.objects.all(), widget=forms.Select(attrs={'style': 'width:265px;', 'class': 'city_search'}))
    city = forms.ModelChoiceField(label=_('City'), help_text=_(
        'Enter your state/region here'), queryset=City.objects.all(), widget=forms.Select(attrs={'style': 'width:265px;'}))

    def save(self, request, *args, **kwargs):
        organization = super(RegistrationForm, self).save(
            False, *args, **kwargs)  # send_email = False
